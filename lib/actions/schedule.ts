...
import { GREG_SCHEDULE_ERROR_MESSAGE, isDemoBlockedTalk, isSentryEmail } from "@/lib/sentry-demo";
      ...
      if (isDemoBlockedTalk(talkId)) {
        ...
        if (isSentryEmail(user?.email)) {
          ...
          Sentry.withScope((scope) => {
            ...
            scope.setTag("demo.scenario", "schedule_block");
            scope.setContext("talk", { id: talkId });
            Sentry.captureException(error);
          });
          Sentry.metrics.count("schedule.event", 1, {
            attributes: { action: "add", result: "demo_blocked" },
          });
          Sentry.logger.warn("Sentry user blocked from adding demo talk", {
            action: "schedule.add",
            result: "demo_blocked",
            user_id: userId,
            talk_id: talkId,
